...
func (payload *Payload) update(r *newPayloadResult, elapsed time.Duration, postFuncs ...func()) {
	...
	if errors.Is(r.err, errInterruptedUpdate) {
		log.Info("Ignoring interrupted payload update", "id", payload.id)
		return
	} else if r.err != nil {
...
func (w *worker) buildPayload(args *BuildPayloadArgs) (*Payload, error) {
	if args.NoTxPool { // don't start the background payload updating job if there is no tx pool to pull from
		...
		log.Warn("try to build empty payload")
	...
	go func() {
		...
		updatePayload := func() time.Duration {
			...
			buildFullPayloadTimer.Update(dur)
			...
			if r.err == nil {
				...
				log.Info("Built updated payload", "id", payload.id, "number", r.block.NumberU64(), "hash", r.block.Hash(), "elapsed", common.PrettyDuration(dur))
			}
