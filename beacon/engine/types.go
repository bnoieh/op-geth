...
func ExecutableDataToBlock(params ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash) (*types.Block, error) {
	...
	log.Error("debug-perf-prefix ExecutableDataToBlock decode", "duration", time.Since(start), "hash", params.BlockHash, "len", len(txs))
	if err != nil {
		return nil, err
	}
	...
	log.Error("debug-perf-prefix ExecutableDataToBlock deriveSha", "duration", time.Since(start), "hash", params.BlockHash, "len", len(txs))
